...
类的元数据（Metadata）：包括类的名称、访问修饰符、常量池、字段描述、方法描述等信息。

静态变量：类中使用static关键字修饰的变量，这些变量在方法区中存储。

常量：包括字符串常量、final常量等。

类的非静态变量信息：即使这些变量是未被实例化的，它们的信息也需要存放在方法区中。
...
需要注意的是，随着Java 8 中引入元空间（Metaspace），方法区的一部分内容，尤其是类的元数据和方法字节码，可能被迁移到元空间中。
